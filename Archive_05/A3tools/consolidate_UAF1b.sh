...
while [ $counter -lt ${count} ]; do
	...
	for avar in ${vars}; do
		...
		ncks -O -d time,-1 ${avar}_tmp.nc ${avar}_tmp1.nc
		ncks -O -d time,-2 ${avar}_tmp.nc ${avar}_tmp2.nc
		# diff
		ncdiff -O ${avar}_tmp1.nc ${avar}_tmp2.nc ${avar}_tmpd.nc
		# rename and add
		ncrename -v ${avar},d${avar} ${avar}_tmpd.nc
		ncks -A  ${avar}_tmpd.nc ${avar}_tmp1.nc
		ncap2 -O -s "${avar}=${avar}+d${avar}" ${avar}_tmp1.nc ${avar}_tmp1.nc
		# change time +360 days
		ncap2 -O -s "time = time+360; time_bnds=time_bnds+360" ${avar}_tmp1.nc ${avar}_tmp1.nc
		# concat
		ncrcat ${avar}_tmp.nc  ${avar}_tmp1.nc ${avar}_full.nc
		# move back in place
		/bin/mv ${avar}_full.nc ${anc}
		# clean up 
		/bin/rm ${avar}_tmp1.nc ${avar}_tmp.nc ${avar}_tmpd.nc ${avar}_tmp2.nc
